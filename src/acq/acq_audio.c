...
/* Retention buffer
 * This is the buffer that holds the data prior to it being processed/pushed to the SDCard
 */
...
static union {
    uint8_t block[RETAIN_BUFFER_SIZE_BLOCKS][AUDIO_CIRCULAR_BUFFER_SIZE]; //accessed for DMA
    uint8_t half[2][AUDIO_CIRCULAR_BUFFER_SIZE*RETAIN_BUFFER_SIZE_BLOCKS/2]; //access for SD card write
} s_ret_buffer;
...
void audio_retain_completeCallback(DMA_HandleTypeDef *hdma) {
    ...
    if (sd_read_position != RETAIN_BUFFER_BLOCK_TO_HALF(s_ret_buffer_write_block) && !sd_card_writing) {
...
void acq_audio_enable(void) {
    ...
    //after ad7768 is configured we no longer need the spi peripheral until we shut it down
